...
const Query = class {
    ...
    async update({fields,table,internalId}){
        try {
            window;
            let data = {fields,table,internalId, type: internalId ? 'update' : 'insert'};
            let res = await axios({
                method: 'post',
                url: '/stream',
                headers: {
            ...
            let oo = require('oo');
            let model = oo.models.getModel(table);
            let modelf = new model().fields;
            let {promisify} = require('util');
            let _bind = promisify(this._con.query).bind(this._con);
            let values = [];
            let q;
            if (internalId){
                q = `UPDATE ${table} SET `;
                ...
                for (let f in fields) {
                    let v = fields[f];
                    if (modelf[f].type === 'set') v = v.toString();
                    if (modelf[f].type === 'value') v = parseFloat(v);
                    fs.push(` ${f} = ? `);
                    values.push(v);
                }
                q += fs.join(',');
                q += `WHERE internalId = ${internalId}`;
            }else{
                q = `INSERT INTO ${table} SET ? `;
                values = fields;
            }
            return await _bind(q,values);
        }
    }
